        ...
        internal override void Execute(Dictionary<string, string> parameters)
        {
            if (this.CliConfig != null)
            {
                if (parameters.ContainsKey(CommandParameters.Name) && !string.IsNullOrWhiteSpace(parameters[CommandParameters.Name]))
                {
                    ...
                    string pagesPath = Path.Combine(this.CliConfigDirectory, "src", "web", this.CliConfig.Base.ProjectName, "Pages");
                    string initialStateRequestsPath = Path.Combine(this.CliConfigDirectory, "src", "web", $"{this.CliConfig.Base.ProjectName}.Application", "Requests", "InitialStates", modifiedPageName);
