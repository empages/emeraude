...
using System.IO;
using Definux.Emeraude.Cli.Commands.Implementations.Request.Templates;
using Definux.Emeraude.Cli.Properties;

namespace Definux.Emeraude.Cli.Commands.Implementations.Request
{
    /// <summary>
        ...
        internal override void Execute(Dictionary<string, string> parameters)
        {
            if (this.CliConfig != null)
            {
                if (parameters.ContainsKey(CommandParameters.Name) && !string.IsNullOrWhiteSpace(parameters[CommandParameters.Name]))
                {
                    ...
                    string requestPath = Path.Combine(this.CliConfigDirectory, "src", "web", $"{this.CliConfig.Base.ProjectName}.Application", "Requests", this.requestsFolderNames[requestType], requestName);
