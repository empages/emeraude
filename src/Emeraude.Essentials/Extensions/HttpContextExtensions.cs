    ...
    public static class HttpContextExtensions
    {
        ...
        public static Guid? GetJwtUserId(this HttpContext httpContext)
        {
            if (httpContext.User.Identity.IsAuthenticated &&
                httpContext.User.Claims.Any(x => x.Type == JwtRegisteredClaimNames.Jti) &&
                Guid.TryParse(httpContext.User.Claims.Where(x => x.Type == JwtRegisteredClaimNames.Jti).Select(x => x.Value).FirstOrDefault(), out var parsedId))
            {
                return parsedId;
            }
        ...
        /// <summary>
        /// <inheritdoc cref="GetCurrentUserId(HttpContext)"/>
        /// </summary>
        /// <param name="httpAccessor"></param>
        /// <returns></returns>
        public static Guid? GetCurrentUserId(this IHttpContextAccessor httpAccessor)
        {
            Guid? resultId = null;
            if (httpAccessor.HttpContext != null)
            {
                resultId = httpAccessor.HttpContext.GetCurrentUserId();
            }
        ...
        /// <summary>
        /// Extract id of the user authenticated with for the current request. User id must be in Guid format.
        /// </summary>
        /// <param name="httpContext"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Returns a flag that indicates whether the request is made from mobile device or not.
        /// </summary>
        /// <param name="httpContext"></param>
        /// <returns></returns>
        public static bool IsMobileDevice(this HttpContext httpContext)
        {
            ...
            return BrowserRegex.IsMatch(userAgent) || BrowserVersionRegex.IsMatch(userAgent.Substring(0, 4));
        }

        /// <summary>
        /// Returns a flag that indicates whether the request user agent is made from mobile device browser or not.
        /// </summary>
        /// <param name="userAgent"></param>
        /// <returns></returns>
        public static bool IsUserAgentMobileDevice(this string userAgent)
        {
            return BrowserRegex.IsMatch(userAgent) || BrowserVersionRegex.IsMatch(userAgent.Substring(0, 4));
        }
        ...
        public static string GetRouteValueOrDefault(this HttpContext httpContext, string routeValueKey, bool normalize = false)
        {
            ...
            route.Values.TryGetValue(routeValueKey, out var routeValueObject);
